...
function SE.onServerMessage(color, text)
	if string.find(text, "^ Ïîäîæäèòå íåìíîãî$") or string.find(text, "^ Âû íå ìîæåòå âçÿòü áîëüøå ïàòðîíîâ äëÿ ýòîãî îðóæèÿ") then
		thisScript():reload()
		return true
	end
	if color == 162529535 and string.find(text, "^ Âûäàíî: Áðîíÿ$") then
...
function SE.onShowDialog(dialogid, style, title, button1, button2, text)
	if dialogid == 1166 and string.find(title, "Ñêëàä îðóæèÿ") and cfg.settings.state then
	...
	elseif dialogid == 123 and string.find(title, "Ñêëàä îðóæèÿ") and cfg.settings.state then
